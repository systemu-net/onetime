...
export default function CampaignsPage() {
  ...
  const [pinnedIds, setPinnedIds] = useState<Set<number>>(() => {
    try {
      const stored = localStorage.getItem("campaigns:pinned");
  ...
  // Client-side filter + pinned-first sort
  const filteredCampaigns = useMemo(() => {
    const query = search.trim().toLowerCase();
    const pinned: Campaign[] = [];
    const rest: Campaign[] = [];
    for (const campaign of campaigns) {
      ...
      (pinnedIds.has(campaign.id) ? pinned : rest).push(campaign);
    }
    return [...pinned, ...rest];
  }, [campaigns, search, stateFilter, typeFilter, pinnedIds]);

  const governedLinks = useMemo(
    () => campaigns.reduce((sum, c) => sum + c.linksCount, 0),
    [campaigns],
  );
        ...
        {!loading && (
          <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            <div className="cc-two-col">
              <div>
                {/* Toolbar — two stable rows, no wrapping */}
                <div className="cc-toolbar-stack">
                  {/* Row 1: search + state filters */}
                  <div className="cc-toolbar-row">
                    <div className="cc-search-wrap shrink-0">
                    ...
                    <div className="cc-filter-scroll">
                      {STATE_FILTERS.map((s) => (
                        <button
                          key={s}
