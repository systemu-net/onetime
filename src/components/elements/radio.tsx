...
const base = [
  ...
  // Focus ring
  'group-data-[focus]:outline group-data-[focus]:outline-2 group-data-[focus]:outline-offset-2 group-data-[focus]:outline-violet-600',
  // Disabled state
...
export function Radio({
  ...
  children,
  ...props
}: { color?: Color; className?: string; children: React.ReactNode } & Omit<Headless.RadioProps, 'as' | 'className' | 'children'>) {
  return (
    <Headless.Radio data-slot="control" {...props} className={clsx(className, 'items-center group inline-flex focus:outline-none cursor-pointer')}>
