...
export function Legend({
  ...
  return (
    <Headless.Legend
      ...
      className={clsx(
        className,
        'text-base/6 text-zinc-850 data-[disabled]:opacity-50 sm:text-sm/6 dark:text-white'
      )}
    />
  )
}
...
export function Title({ className, ...props }: { className?: string } & Omit<Headless.LabelProps, 'as' | 'className'>) {
  return (
    <Headless.Label
      data-slot="label"
      {...props}
      className={clsx(
        className,
        'select-none font-semibold text-base/6 text-zinc-950 data-[disabled]:opacity-50 dark:text-white'
      )}
    />
  )
}
