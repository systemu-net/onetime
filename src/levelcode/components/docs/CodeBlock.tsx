...
export default function CodeBlock({ children, ...props }: ComponentPropsWithoutRef<"figure">) {
  ...
  // `text-[#abb2bf]` is the pane's DEFAULT foreground — One Dark's own. It matters
  // for unhighlighted fences: a ```text block gets no colour spans from Shiki, so
  // without this its characters inherit the page body colour (--c-text2, #555 in
  // the light theme) and are all but invisible on this near-black pane. Highlighted
  // tokens carry inline colours and override it.
  //
  // Note on the [&_code]: resets — the `code` mapping in mdxComponents styles
  // INLINE code (border, padding, tinted background, and text colour). MDX maps by
  // tag name with no notion of nesting, so it also lands on the <code> inside a
  // fence; these strip it back off. `text-inherit` is the important one: without
  // it the inline colour (--c-text, #333 in the light theme) wins over the pane's
  // foreground and an unhighlighted fence renders near-black on near-black.
  return (
    <figure
      ref={ref}
      {...props}
      className="group relative my-5 overflow-hidden rounded-md border border-black/20 bg-[#0d1321] text-[#abb2bf] [&_code]:grid [&_code]:rounded-none [&_code]:border-0 [&_code]:bg-transparent [&_code]:p-0 [&_code]:font-mono [&_code]:text-[13.5px] [&_code]:leading-[1.65] [&_code]:text-inherit [&_pre]:overflow-x-auto [&_pre]:bg-transparent [&_pre]:py-4 [&_[data-line]]:px-4"
    >
